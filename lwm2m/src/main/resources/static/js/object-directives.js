...
.directive('object', function ($compile, $routeParams, $http, dialog, $filter, $modal, lwResources, helper, uriBase) {
    return {
        restrict: "E",
        replace: true,
        scope: {
            ...
            scope.create = function () {
                ...
                modalInstance.result.then(function (result) {
                    ...
                    Promise.all(promisedValues).then(function(resourceValues) {
                        ...
                        $http({method: 'POST', url: uriBase + "/clients/" + encodeURIComponent($routeParams.clientId) + instancepath, data: payload, headers:{'Content-Type': 'application/json'}, params:{format:format,timeout:timeout}})
