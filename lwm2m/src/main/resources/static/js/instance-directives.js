...
.directive('instance', function ($compile, $routeParams, $http, dialog,$filter, lwResources, $modal, helper, uriBase) {
    return {
        restrict: "E",
        replace: true,
        scope: {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.read = function() {
                ...
                var uri = uriBase + "/clients/" + encodeURIComponent($routeParams.clientId) + scope.instance.path;
            ...
            scope.write = function () {
                ...
                modalInstance.result.then(function (result) {
                    ...
                    Promise.all(promisedValues).then(function(resourceValues) {
                        ...
                        var format = scope.settings.multi.format;
                        var timeout = scope.settings.timeout.value;
                        $http({method: 'PUT', url: uriBase + "/clients/" +encodeURIComponent($routeParams.clientId) + scope.instance.path, data: payload, headers:{'Content-Type': 'application/json'}, params:{format:format, replace:result.replace, timeout:timeout}})
                        .success(function(data, status, headers, config) {
            ...
            scope.startObserve = function() {
                var format = scope.settings.multi.format;
                var timeout = scope.settings.timeout.value;
                var uri = uriBase + "/clients/" + encodeURIComponent($routeParams.clientId) + scope.instance.path+"/observe";
