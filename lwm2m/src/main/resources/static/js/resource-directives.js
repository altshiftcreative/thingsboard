...
.directive('resource', function ($compile, $routeParams, $http, dialog, $filter, lwResources, $modal, helper, uriBase) {
    return {
        restrict: "E",
        replace: true,
        scope: {
        ...
        link: function (scope, element, attrs) {
            ...
            scope.startObserve = function() {
                ...
                var uri = uriBase + "/clients/" + encodeURIComponent($routeParams.clientId) + scope.resource.path+"/observe";
            ...
            scope.read = function() {
                var timeout = scope.settings.timeout.value;
                var format = scope.settings.single.format;
                var uri = uriBase + "/clients/" + encodeURIComponent($routeParams.clientId) + scope.resource.path;
            ...
            scope.write = function() {
                ...
                modalResource.result.then(function (resource) {
                    resource.getPromisedValue().then(function(resourceValue){
                        ...
                        var format = scope.settings.single.format;
                        var timeout = scope.settings.timeout.value;
                        $http({method: 'PUT', url: uriBase + "/clients/" + encodeURIComponent($routeParams.clientId) + scope.resource.path, data: payload, headers:{'Content-Type': 'application/json'},params:{format:format, timeout:timeout}})
                        .success(function(data, status, headers, config) {
            ...
            scope.exec = function() {
                var timeout = scope.settings.timeout.value;
                $http({method:'POST', url:uriBase + "/clients/" + encodeURIComponent($routeParams.clientId)+ scope.resource.path, params:{timeout:timeout}})
                .success(function(data, status, headers, config) {
            ...
            scope.execWithParams = function() {
                ...
                $('#writeSubmit').click(function(e){
                    ...
                    if(value) {
                        ...
                        var timeout = scope.settings.timeout.value;
                        $http({method: 'POST', url: uriBase + "/clients/" + encodeURIComponent($routeParams.clientId) + scope.resource.path, data: value, params:{timeout:timeout}})
                        .success(function(data, status, headers, config) {
                            helper.handleResponse(data, scope.resource.exec);
                        }).error(function(data, status, headers, config) {
                            if (status == 504){
